...
public class PersonVisualizer 
{
  ...
  public  JComponent visualize(final PluginContext context,
	...
	//Scrolls  
	  ...
	  topContainer.setSize(300,200);
	  topContainer.add(new JLabel("Nuevo Plugin"));
	  topContainer.setBackground(Color.LIGHT_GRAY);
		JScrollPane scp = new JScrollPane(topContainer, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
				JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
		JPanel graphPanel= new JPanel();
		JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, scp, graphPanel);
	...
	JLabel Nombre= new JLabel(person.getName().getFirst() + " " + person.getName().getLast());	
	JButton button = new JButton();
		button.setText("Project");
		button.repaint();
	graphPanel.setSize(new Dimension(final_view.getWidth()/2,200));
    graphPanel.add(Nombre);
	...
	final_view.setBorder(title);
	final_view.setBackground(new Color(30, 30, 30));
	final_view.setSize(new Dimension(final_view.getWidth(), 400));
	final_view.setPreferredSize(new Dimension(final_view.getWidth(), 30));
	final_view.setMinimumSize(new Dimension(final_view.getWidth(), 30));
    
	final_view.add(scp);
	//final_view.add(splitPane, BorderLayout.CENTER);
	final_view.add(graphPanel);
	final_view.add(topContainer);
	fuzzyview();
