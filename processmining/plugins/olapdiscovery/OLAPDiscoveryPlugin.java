...
/**
 *  A plugin to make a discovery with OLAP logic, is a tool to discovery and analyze at the same time.
 * It is for Business process users 
 *  @author Gustavo Pizarro
*/


@Plugin(name = "OLAPDiscovery",
        parameterLabels = { "Log", "None", "OLAPDiscovery Configuration" },
...
public class OLAPDiscoveryPlugin {
  ...
  public static OLAPDiscoveryConfiguration Ini(final UIPluginContext context,final OLAPDiscoveryConfiguration config) 
  {
	  
	ProMPropertiesPanel panel = new ProMPropertiesPanel("OLAPDiscovery Configuration");
	ProMTextField name = panel.addTextField("Name", config.getName());
	final InteractionResult interactionResult = context.showConfiguration("Setup OLAP", panel);
